...
static Real alpha, beta, rho0, P0, pcoeff, vr, vphi, perturb, mphi;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  vphi= pin->GetOrAddReal("problem", "vphi", 0.0);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int j=js; j<=je+1; ++j) {
    for (int i=is; i<=ie+1; ++i) {
      ...
      //az(j,i)=(bphi0/(beta+1))*pow(rad,beta+1)*(1+perturb*sin(mphi*rad)/(mphi*rad));//beres
    }
  }
      ...
      for (int i=is; i<=ie; i++) {
        // Volume centered coordinates and quantities
         ...
         if(COORDINATE_SYSTEM == "cylindrical") {
             ...
             phydro->u(IM1,k,j,i) = rho*vr*x1/rad+rho*vphi*x2/rad;
             phydro->u(IM2,k,j,i) = rho*vr*x2/rad-rho*vphi*x1/rad;
         }

         phydro->u(IM3,k,j,i) = 0.0;
         phydro->u(IEN,k,j,i) = P/(peos->GetGamma()-1.0) + 0.5*rho*SQR(vr)+0.5*rho*SQR(vphi);
      }
    }
  }
